...
def main():
  ...
  for split in SPLITS:
    ...
    for sample in nusc.sample:
      ...
      ########################
      if DEBUG:
          rpath = '/home/ubuntu/data/YoungseokKim/DataSet/nuscenes/v1.0-trainval/radar/%06d.bin' % idx_local
          radar = np.fromfile(rpath, dtype=np.float32).reshape(-1, 6)
          fig, ax = plt.subplots(figsize=(12, 6))
          ax.plot(radar[:,0], radar[:,1], ',')
          plt.xlim([-60, 60])
          plt.ylim([-60, 60])
