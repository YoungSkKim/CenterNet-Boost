...
class GenericDataset(data.Dataset):
  ...
  def _get_depth_anno(self, depth, ret, trans_input, aug_s, flipped):
    if flipped:
      depth = depth[:, ::-1].copy()
    ...
    if self.opt.eval_depth:
      size_h, size_w = self.opt.input_h, self.opt.input_w
      self.opt.auxdep_ratio = 1.0
      self.opt.auxdep_mask = True
    else:
      size_h, size_w = self.opt.output_h, self.opt.output_w

    mask = np.random.uniform(0, 1, size=size_h*size_w).reshape(size_h, size_w)
    mask = mask <= self.opt.auxdep_ratio

    auxdep_mask = np.zeros((size_h, size_w), dtype=np.float32)
    auxdep_mask_border = 0.05 if not self.opt.eval_depth else 0.0

    num_objs = len(ret['mask'][ret['mask'] > 0])
    if self.opt.auxdep_mask and num_objs > 0:
      bbox = ret['bboxes'][:num_objs, :].copy()
      if self.opt.eval_depth:
        bbox = bbox*4
      wh = auxdep_mask_border * ret['wh'][:num_objs, :].copy()
      bbox += np.hstack((-wh, wh))
      bbox[:, 0] = np.maximum(bbox[:, 0], np.zeros(num_objs))
      bbox[:, 2] = np.minimum(bbox[:, 2], np.ones(num_objs)*size_w)
      bbox[:, 1] = np.maximum(bbox[:, 1], np.zeros(num_objs))
      bbox[:, 3] = np.minimum(bbox[:, 3], np.ones(num_objs)*size_h)
      bbox = bbox.astype(np.int32)
      for i in range(num_objs):
        auxdep_mask[bbox[i, 1]:bbox[i, 3], bbox[i, 0]:bbox[i, 2]] = 1

    mask = np.logical_or(mask, auxdep_mask)
    depth_mask = np.logical_and(mask, depth_mask, dtype=np.float32)
    if self.opt.eval_depth:
      depth_auxdep_mask = np.logical_and(depth_mask, auxdep_mask, dtype=np.float32)
      depth_mask = np.dstack((depth_mask, depth_auxdep_mask))
