...
class Detector(object):
  ...
  def run(self, image_or_path_or_tensor, meta={}, id=None):
  ...
  def debug(self, debugger, images, dets, output, scale=1,
    ...
    debugger.add_blend_img(img, self.pred, 'pred_hm')
    if 'hm_hp' in output:
  ...
  def show_results(self, debugger, image, results):
    ...
    for j in range(len(results)):
      if results[j]['score'] > self.opt.vis_thresh:
        ...
        if ('bbox' in item):
          ...
          debugger.add_coco_bbox(  # visualize score
            item['bbox'], item['class'] - 1, sc, no_bbox=True, img_id='generic')
    ...
    if len(results) > 0 and \
      ...
      debugger.add_bird_view(
        ...
        img_id='bird_pred', cnt=self.id)
      debugger.compose_vis_ddd(
        debugger.imgs['generic'], False, results, self.this_calib,
        self.opt.vis_thresh,
        self.pred, 'bird_pred', img_id='out')
      # for dets in results:
      #   debugger.add_coco_bbox(
      #     dets['bbox'] * self.opt.down_ratio, dets['class'],
      #     dets['score'], img_id='out')
    ...
    if self.opt.debug == 4:
      debugger.save_all_imgs(self.opt.debug_dir, prefix='{}'.format(self.id))
    else:
