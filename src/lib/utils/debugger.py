...
class Debugger(object):
  def __init__(self, opt, dataset):
    ...
    self.out_size = 384 if opt.dataset == 'kitti' else 448
    self.cnt = 0
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, 
    ...
    if conf >= 1:
      ...
      txt = '{:.2f}'.format(conf)  # display score only
    thickness = 2
    fontsize = 0.4 if self.opt.qualitative else 0.5
  ...
  def save_all_imgs(self, path='./cache/debug/', prefix='', genID=False):
    ...
    for i, v in self.imgs.items():
      if i in self.opt.save_imgs or self.opt.save_imgs == []:
        cv2.imwrite(
          path + '/{}_{}{}.png'.format(i, prefix, self.opt.save_img_suffix), v)
  ...
  def add_3d_detection(
    ...
    # print('===========================')
  def compose_vis_ddd(
    self, image_or_path, flipped, dets, calib,
    ...
    for item in dets:
      if item['score'] > vis_thresh:
        ...
        if loc[2] > 1:
          ...
          box_2d[:, 0] /= hs
          box_2d[:, 1] /= ws
          if any(box_2d[:, 0] < -1000) or any(box_2d[:, 1] < -1000): continue
  ...
  def add_bird_view(self, dets, vis_thresh=0.3, img_id='bird', cnt=0):
    if self.opt.vis_gt_bev:
      bird_view = cv2.imread(
        self.opt.vis_gt_bev + '/bird_pred_gt_{}.png'.format(cnt))
    else:
